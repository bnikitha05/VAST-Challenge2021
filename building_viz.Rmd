...
**Overall Distribution of Emails:**
...
**Official and Unofficial Relationships:**
...
**Unofficial Relationships related to 'Politics', 'Kronos'and 'Media':**
...
unofficial_discussions$filter.condition[i] <-ifelse(ifelse(any(str_detect(unofficial_discussions$Subject[i],keyterms))==TRUE,TRUE,FALSE),TRUE,FALSE)
}

unofficial_discussions=unofficial_discussions %>% 
  filter(filter.condition == TRUE)

DT::datatable(unofficial_discussions)
```
...
g + theme_graph()
```
**Role of Citizenship in Employment Type:**
